...
export default function LocationsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!formData.name.trim() || !formData.address.trim()) {
        setError('Navn og adresse er påkrævet');
        setSubmitting(false);
        return;
      }
      ...
      setError(err.message || 'Der opstod en fejl ved oprettelse af lokation');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (locationId: string) => {
    ...
    try {
      ...
      alert(err.message || 'Kunne ikke slette lokation');
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold mb-2">Lokationer</h1>
          <p className="text-muted-foreground">
            Administrer dine virksomhedslokationer
          </p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Tilføj en ny lokation hvor arbejdet kan udføres
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit}>
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  <Label htmlFor="name">
                    Lokationsnavn <span className="text-red-500">*</span>
                  </Label>
                  <Input
                    id="name"
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    placeholder="F.eks. Hovedlager, City Restaurant"
                    required
                    disabled={submitting}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="F.eks. Hovedgade 123, 2100 København"
                    required
                    disabled={submitting}
                  />
                </div>
